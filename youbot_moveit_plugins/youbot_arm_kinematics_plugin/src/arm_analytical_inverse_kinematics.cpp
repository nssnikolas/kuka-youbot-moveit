...
{
	ROS_DEBUG("ArmAnalyticalInverseKinematics::CartToJnt\n");
...
void ArmAnalyticalInverseKinematics::getSolverInfo(moveit_msgs::KinematicSolverInfo &info)
{
	info = _solver_info;
}
		...
		bool offset_joint_1, bool offset_joint_3, bool offset_joint_5)
{
	...
	if ((l2 + l3) < sqrt((p2.x() * p2.x()) + (p2.z() * p2.z()))) {
		ROS_DEBUG("Goal position out of reach: len (%f) < sqrt(x + z). p2.x = %f, p2.z = %f", (l2 + l3), p2.x(), p2.z());
		return KDL::JntArray();
	}
