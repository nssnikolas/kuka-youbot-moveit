	...
	const std::string &root_name)
{
	...
	while (link) {
		...
		// process the joint
		std::shared_ptr<const urdf::Joint> joint = robot_model.getJoint(link->parent_joint->name);
		if (!joint) {
...
moveit_msgs::KinematicSolverInfo SolverInfoProcessor::getSolverInfo() const
{
	return _solver_info;
}


void SolverInfoProcessor::addJointToChainInfo(std::shared_ptr<const urdf::Joint> joint, moveit_msgs::KinematicSolverInfo &info)
{
